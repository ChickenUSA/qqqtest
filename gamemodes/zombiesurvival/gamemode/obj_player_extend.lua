...
function meta:GetPointsThisRound()
	return self:GetDTInt(2)
end

function meta:GetStartingPoints()
...
function meta:ResetSpeed(noset, health)
	...
	if P_Team(self) == TEAM_UNDEAD then
		local speed = (math.max(140, self:GetZombieClassTable().Speed * GAMEMODE.ZombieSpeedMultiplier - (GAMEMODE.ObjectiveMap and 20 or 0))) * (self.ZombieSpeedMultiplier or 1)
...
function meta:GetMaxZombieHealth()
	return self:GetZombieClassTable().Health * (self.ZombieHealthMultiplier or 1)
end
...
function meta:GetZombieMeleeSpeedMul()
	return 1 * (1 + math.Clamp(self:GetArmDamage() / GAMEMODE.MaxArmDamage, 0, 1)) / ((self:GetStatus("zombie_battlecry") and 1.2 or 1) * (self.ZombieAttackMultiplier or 1))
end
...
function meta:GetMeleeSpeedMul()
	...
	return 1 * (1 + math.Clamp(self:GetArmDamage() / GAMEMODE.MaxArmDamage, 0, 1)) / (self:GetStatus("frost") and 0.7 or 1) * (self.MeleeSwingSpeedMul or 1)
end
