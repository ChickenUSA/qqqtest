...
function ENT:Think()
	...
	if self:GetIsHeavy() then
		if self:GetHingePos():DistToSqr(self:GetPullPos()) >= 20736 then
			self:Remove()
			return
		end
	elseif nearestpoint:DistToSqr(shootpos) >= 28561 then
		self:Remove()
		return
	end
	...
	elseif owner:KeyDown(IN_ATTACK2) and not owner:GetActiveWeapon().NoPropThrowing then
		...
		if owner:KeyDown(IN_SPEED) then
			if owner:KeyDown(IN_WALK) then
				...
				local sxdiff = xdiff * FrameTime() * 23 --rotation sentintivy 
				local sydiff = ydiff * FrameTime() * 23 --rotation senesitnvity 

				self.ObjectAngles:RotateAroundAxis(owner:GetUp(), sxdiff)
				self.ObjectAngles:RotateAroundAxis(owner:GetRight(), sydiff)
				self.ObjectAngles:SnapTo( "p", 45 ):SnapTo( "y", 45 ):SnapTo( "r", 45 ) --rotation angle
		...
		elseif owner:KeyDown(IN_WALK) then
			local xdiff = math.NormalizeAngle(self.StartX - (owner.InputMouseX or 0))
			local ydiff = math.NormalizeAngle(self.StartY - (owner.InputMouseY or 0))
			local sxdiff = xdiff * FrameTime() * 8 --rotation sensitivity
