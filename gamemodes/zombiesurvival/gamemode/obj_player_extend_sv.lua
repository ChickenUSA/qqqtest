...
function meta:SetPointsThisRound(points)
	self:SetDTInt(2, points)
end

function meta:SetStartingPoints(points)
...
function meta:ScrapCashOut()
	if self.ScrapQueue >= 1 and P_Team(self) == TEAM_HUMAN then
		local scraps = math.floor(self.ScrapQueue)
		self.ScrapQueue = self.ScrapQueue - scraps
		
		self:GiveAmmo(scraps, "Scrap")
	end
end
...
function meta:AddPoints(points, floatingscoreobject, fmtype, nomul)
	...
	self:SetPointsThisRound(self:GetPointsThisRound() + wholepoints)
...
function meta:StopNoise()
	local rf = RecipientFilter()
	rf:AddPAS(self:GetPos())
	net.Start("voice_stopnoise")
	net.WriteEntity(self)
	net.Send(rf)
end
