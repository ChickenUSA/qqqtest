	...
	if attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD then
		...
		if self:HumanNearby() then
			...
			if not attacker:IsBot() then
				attacker:AddPoints(math.ceil(dmg/20))
			end
		end
	end
end
...
function meta:DamageNails(attacker, inflictor, damage, dmginfo)
	...
	for i, nail in ipairs(nails) do
		...
		if NailOwner and NailOwner:IsValid() and NailOwner:IsPlayer() then
			NailOwner:AddPoints(damage/100)
			NailOwner.ScrapQueue = NailOwner.ScrapQueue + (damage/500)
			--NailOwner:GiveAmmo(math.ceil(damage/50, "Scrap"))
