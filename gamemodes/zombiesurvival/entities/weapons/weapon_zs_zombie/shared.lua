...
function SWEP:Swung()
	...
	if prehit then
		...
		if ins then
			...
			if prehit.Entity:IsValid() and prehit.Entity:NearestPoint(eyepos):DistToSqr(eyepos) <= self.MeleeReach * self.MeleeReach * (owner.ZombieRangeMultiplier or 1) * (owner.ZombieRangeMultiplier or 1) then
...
function SWEP:StartSwinging()
	...
	if self.MeleeDelay > 0 then
		...
		local trace = owner:CompensatedMeleeTrace(self.MeleeReach * (owner.ZombieRangeMultiplier or 1), self.MeleeSize)
...
function SWEP:Deploy()
	...
	if self.DelayWhenDeployed and self.Primary.Delay > 0 then
		self:SetNextPrimaryFire(CurTime() + self.Primary.Delay / (owner.ZombieAttackMultiplier or 1))
		self:SetNextSecondaryFire(self:GetNextPrimaryFire() + 0.5)
	end
