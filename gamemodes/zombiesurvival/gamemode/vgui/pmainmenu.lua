...
function GM:ShowHelp()
	...
	but:SetFont("ZSHUDFontSmaller")
	but:SetText("Vote Map")
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	but.DoClick = function() RunConsoleCommand("glmvs_openvote") end

	--but = vgui.Create("DButton", menu)
	--but:SetFont("ZSHUDFontSmaller")
	--but:SetText("Hand Selector")
	--but:SetTall(buttonhei)
	--but:DockMargin(0, 0, 0, 12)
	--but:DockPadding(0, 12, 0, 12)
	--but:Dock(TOP)
	...
	but = vgui.Create("DButton", menu)
	but:SetFont("ZSHUDFontSmaller")
	but:SetText("Point Shop")
	but:SetTall(buttonhei)
	but:DockMargin(0, 0, 0, 12)
	but:DockPadding(0, 12, 0, 12)
	but:Dock(TOP)
	but.DoClick = function() RunConsoleCommand("PS_OPEN") end
