...
function SWEP:SpawnGhost()
	...
	if owner and owner:IsValid() then
		owner:GiveStatus("ghost_propvendor")
	end
end
...
function SWEP:RemoveGhost()
	local owner = self:GetOwner()
	if owner and owner:IsValid() then
		owner:RemoveStatus("ghost_propvendor", false, true)
	end
end
...
function SWEP:PrimaryAttack()
	...
	local status = owner.status_ghost_propvendor
	...
	local ent = ents.Create("prop_propvendor")
	if ent:IsValid() then
		ent:SetPos(pos)
		ent:SetAngles(ang)
		ent:Spawn()
