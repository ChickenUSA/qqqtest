...
function PANEL:Text1Paint()
	...
	if override and #override > 0 then
		...
		elseif GAMEMODE.ZombieEscape then
			...
			-- Wagon Fish: I'm so upping this, first add poll.
				max_rounds = GAMEMODE.RoundLimit
				round = GAMEMODE.CurrentRound
				text = text .. " - " .. translate.Format("round_x_of_y", round, max_rounds)
			--end
		else
...
function PANEL:Text3Paint()
	if MySelf:IsValid() then
		if MySelf:Team() == TEAM_UNDEAD then
			...
			draw.SimpleText("Points: "..MySelf:GetPoints().." / "..MySelf:GetPointsThisRound().." / "..MySelf:GetPoints() - MySelf:GetStartingPoints(), self.Font, 0, 0, COLOR_SOFTRED)
		end
	end
