--[[
script hookers are fags and you should kill yourself immediately
]]
...
function GM:AddNetworkStrings()
	...
	util.AddNetworkString("voice_stopnoise")
end
...
function GM:ShowSpare2(pl)
	if pl:Team() == TEAM_HUMAN then
		pl:SendLua("GAMEMODE:OpenRemantlerMenu(MySelf:NearestRemantler())")
	else
...
function GM:Think()
	...
	for _, pl in pairs(allplayers) do
		...
		if P_Team(pl) == TEAM_HUMAN then
			...
			if pl.ScrapQueue >= 1 and time >= pl.LastDamageDealtTime + 2 then
				pl:ScrapCashOut()
			end
...
function GM:PlayerHealedTeamMember(pl, other, health, wep, pointmul, nobymsg, floater)
	...
	if pointmul ~= 0 then
		...
		pl:AddPoints(points*12)
		pl.ScrapQueue = pl.ScrapQueue + (points * 12)
	end
...
function GM:PlayerRepairedObject(pl, other, health, wep)
	...
	pl:AddPoints(points)
	pl.ScrapQueue = pl.ScrapQueue + (points * 2.6)
...
function GM:EndRound(winner)
	...
	--MapVote.Start(voteLength, allowCurrentMap, mapLimit, mapPrefix) -- re enable this if we switch back to the ttt map vote
...
function GM:EntityTakeDamage(ent, dmginfo)
	...
	if attacker:IsValid() and attacker:IsPlayer() and attacker:Team() == TEAM_UNDEAD and attacker.ZombieDamageMultiplier then
		dmginfo:SetDamage(dmginfo:GetDamage() * attacker.ZombieDamageMultiplier)
	end
				...
				if myteam ~= otherteam then
					...
					if damage > 0 then
						...
						elseif myteam == TEAM_HUMAN and otherteam == TEAM_UNDEAD then
							damage = damage / (ent.ZombieResistanceMultiplier or 1)
							...
							if time >= ent.m_LastWaveStartSpawn + 3 and time >= ent.m_LastGasHeal + 2 then
								...
								if GAMEMODE:GetWave() > 0 then
									if GAMEMODE:GetWave() > 5 then
										attacker.ScrapQueue = attacker.ScrapQueue + (damage/113)
									elseif GAMEMODE:GetWave() > 4 then
									 attacker.ScrapQueue = attacker.ScrapQueue + (damage/110)
									elseif GAMEMODE:GetWave() > 3 then
									 attacker.ScrapQueue = attacker.ScrapQueue + (damage/107)
									elseif GAMEMODE:GetWave() > 2 then
									 attacker.ScrapQueue = attacker.ScrapQueue + (damage/104)
									elseif GAMEMODE:GetWave() > 1 then
									 attacker.ScrapQueue = attacker.ScrapQueue + (damage/101)
									elseif GAMEMODE:GetWave() < 2 then
									 attacker.ScrapQueue = attacker.ScrapQueue + (damage/98)
									end
								end
	...
	elseif entclass == "func_door_rotating" then
		...
		if attacker:IsPlayer() and P_Team(attacker) == TEAM_HUMAN then
			dmginfo:SetDamage(0)
			dmginfo:SetDamageType(0)
			dmginfo:ScaleDamage(0)
			dmginfo:SetDamageForce(vector_origin)
...
function GM:OnPlayerChangedTeam(pl, oldteam, newteam)
	...
	elseif newteam == TEAM_HUMAN then
		...
		if self.PointSaving > 0 and pl.PointsVault ~= nil and not self.ZombieEscape and not self:IsClassicMode() then
			pl:SetStartingPoints(math.floor(pl.PointsVault))
			pl:SetPoints(math.floor(pl.PointsVault))
		else
			pl:SetPoints(0)
			pl:SetStartingPoints(0)
		end
...
function GM:HumanKilledZombie(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	local zombieClass = pl:GetZombieClassTable().Name
	if attacker:HasTrinket("lifesteal") and zombieClass ~= "Crow" and zombieClass ~= "Gore Child" then
		local distanceFromZombie = attacker:GetPos():Distance(pl:GetPos())
		local healFormula = 0
		if distanceFromZombie < 2048 then
			attacker:SetHealth(math.min(attacker:GetMaxHealth(), attacker:Health() + math.random(0,4)))
		end
	end
...
function GM:ZombieKilledHuman(pl, attacker, inflictor, dmginfo, headshot, suicide)
	...
	local xp = 100 * (GAMEMODE.ZombieXPMulti or 1)
	...
	attacker:AddPoints(self.InitialVolunteers[attacker:UniqueID()] and xp or math.floor(xp/4))
...
function GM:DoPlayerDeath(pl, attacker, dmginfo)
	...
	if not pl:CallZombieFunction5("OnKilled", attacker, inflictor, suicide, headshot, dmginfo) then
		if pl:Health() <= -70 and not pl.NoGibs and not self.ZombieEscape or dmginfo:IsDamageType(DMG_ALWAYSGIB) then
			pl:Gib(dmginfo)
	...
	if plteam == TEAM_UNDEAD then
		...
		if (pl.Gibbed or inflictor.NoScream or headshot) then
			pl:StopNoise()
		else
	...
	elseif plteam == TEAM_HUMAN then
		...
		if (pl.Gibbed or headshot or inflictor.NoScream) then
			pl:StopNoise()
		else
			pl:PlayDeathSound()
		end
...
function GM:PlayerSpawn(pl)
	...
	if pl:Team() == TEAM_UNDEAD then
		...
		if classtab.Boss then
			pl:SetHealth(classtab.Health * (pl.ZombieHealthMultiplier or 1))
		else
			local lowundead = team.NumPlayers(TEAM_UNDEAD) < 4

			local healthmulti = (self.ObjectiveMap or self.ZombieEscape) and 1 or lowundead and 1.5 or pl.ZombieHealthMultiplier or 1
	...
	if #UnlockedClasses > 0 then
		for _, pl in pairs(player.GetAll()) do
			...
			net.Start("zs_classunlock")
				net.WriteString(#classnames) --string.AndSeparate(classnames))
			net.Send(pl)
		end
	end
end
			...
			if pl:Team() == TEAM_HUMAN and pl:Alive() then
				...
				if pointsbonus then
					...
					if pl:IsSkillActive(SKILL_SCOURER) then
						pl.ScrapQueue = attacker.ScrapQueue + (pointsreward)
					else
