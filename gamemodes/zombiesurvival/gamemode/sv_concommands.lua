...
concommand.Add("zs_pointsshopbuy", function(sender, command, arguments)
	...
	if usescrap and not sender:NearRemantler() then
		GAMEMODE:ConCommandErrorMessage(sender, translate.ClientGet(sender, usescrap and "need_to_be_near_remantler"))
		return
	end
...
concommand.Add("zs_dismantle", function(sender, command, arguments)
	...
	if not invitem then
		...
		if wtbl.NoDismantle or not (wtbl.AllowQualityWeapons or wtbl.PermitDismantle) or not wtbl.QualityTier then
