...
function meta:ApplySkills(override)
	...
	local desired = override or  self:GetDesiredActiveSkills() or {}
	...
	for skillid in pairs(allskills) do
		...
		if funcs then
			if current_active[skillid] and not desired_assoc[skillid] --[[or not ((self:Team() == TEAM_UNDEAD) == (allskills[skillid].tree == TREE_ZOMBIETREE))]] then -- On but we want it off.
				for _, func in pairs(funcs) do
					func(self, false)
				end
			elseif desired_assoc[skillid] --[[and ((self:Team() == TEAM_UNDEAD) == (allskills[skillid].tree == TREE_ZOMBIETREE))]] and not current_active[skillid] then -- Off but we want it on.
				for _, func in pairs(funcs) do
					func(self, true)
				end
