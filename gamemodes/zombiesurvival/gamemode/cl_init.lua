...
function GM:HumanHUD(screenscale)
	...
	if not self.RoundEnded then
		...
		--surface.DrawRect(x, y - 4, width, 2)
		--surface.DrawRect(x, y - height, 2, height - 4)


		--surface.DrawTexturedRect(x + width, y - height - 2, 48, height)
		--surface.DrawTexturedRect(x + width, y - height - 2, 48, 2)
		--surface.DrawTexturedRect(x + width, y - 4, 48, 2)

		draw.SimpleTextOutlined("Scrap:", "ZSHUDFontSmall", x + 5, y - margin - 2, Color(255, 255, 255, 200), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 2, Color(50, 50, 50, 255))
		
		surface.SetFont("ZSHUDFontSmall")
		local texW, texH = surface.GetTextSize("Scrap:")
		draw.SimpleTextOutlined(MySelf:GetAmmoCount("Scrap"), "ZSHUDFontSmall", x + texW + 15, y - margin - 2, Color(255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 2, Color(50, 50, 50, 255))
	...
	if gamemode.Call("PlayerCanPurchase", MySelf) and not self:GetWaveActive() then
