...
net.Receive("zs_skills_nextreset", function(length)
	...
	if GAMEMODE.SkillWeb and GAMEMODE.SkillWeb:IsValid() then
		...
		btn:SetDisabled(--[[GAMEMODE.NextSkillReset > 0]] false)
	end
end)
...
local REMORT_SKILL = {Name = "Prestige", Description = "Gain 10,000 Points and 1 skill point, \nin exchange for all of your xp."}
...
local offsets = {
	[TREE_HEALTHTREE] = {-10, 14},
	[TREE_SPEEDTREE] = {14, 0},
	[TREE_GUNTREE] = {12, -16},
	[TREE_MELEETREE] = {10, 14},
	[TREE_BUILDINGTREE] = {-14, -1},
	[TREE_SUPPORTTREE] = {-12, -16},
	[TREE_ZOMBIETREE] = {0,-12}
}
	...
	if mc == MOUSE_LEFT then
		...
		if hoveredskill then
			...
			if hoveredskill == -1 and can_remort then
				Derma_Query(
					"Gain 10,000 Points and 1 skill point, \nin exchange for all of your xp.",
					"Warning",
					"OK",
...
function PANEL:PerformLayout()
	...
	self:SetSize(420 * screenscale, 36 * screenscale)
