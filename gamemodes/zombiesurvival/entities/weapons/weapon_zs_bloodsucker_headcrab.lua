...
function SWEP:Think()
	...
	elseif self:GetPouncing() then
		if owner:IsOnGround() or 1 < owner:WaterLevel() then
			...
			self:SetNextPrimaryFire(curtime + self.NoHitRecovery / (owner.ZombieAttackMultiplier or 1))
		else
			...
			if trace.Hit then
				self:SetPouncing(false)
				self:SetNextPrimaryFire(curtime + self.HitRecovery / (owner.ZombieAttackMultiplier or 1))
			end
