...
local branch = GAMEMODE:AddNewRemantleBranch(SWEP, 2, "'Röntgen' Ion Caster", "Shoots irradiated particles that deals area damage but do not slow", function(wept)	
	function wept.BulletCallback(attacker, tr, dmginfo)
		local ent = tr.Entity
			
		if ent:IsValidZombie() then
			util.BlastDamageEx(attacker:GetActiveWeapon(), attacker, ent:GetPos(), 88, wept.Primary.Damage/5, DMG_DISSOLVE, 0.94)
		end
