...
function meta:PS_PlayerDeath()
	...
	self:SetModel("models/player/zombie_classic_hbfix.mdl")
...
--	self.PS_Points = 0
	...
	if PS.Config.NotifyOnJoin then
		if PS.Config.ShopKey ~= '' then
			timer_Simple(5, function() -- Give them time to load up
				if not IsValid(self) then return end
		...
		timer_Simple(10, function() -- Give them time to load up
			if not IsValid(self) then return end
			self:PS_Notify('You have ' .. self:GetPoints() .. ' ' .. PS.Config.PointsName .. ' to spend!')
		end)
	end
...
function meta:PS_CanPerformAction(itemname)
	local allowed = true
	...
	if self:Team() ~= TEAM_HUMAN then allowed = false end
...
function meta:PS_GiveItem(item_id)
...
function meta:PS_TakeItem(item_id)
	if not PS.Items[item_id] then return false end
...
function meta:PS_BuyItem(item_id)
	...
	if ITEM.AdminOnly and not self:IsUserGroup('developer') then
		self:PS_Notify('This item is Developer only!')
		return false
	end
...
function meta:PS_AddClientsideModel(item_id)
	...
	net_Start('PS_AddClientsideModel')
...
function meta:PS_RemoveClientsideModel(item_id)
	...
	net_Start('PS_RemoveClientsideModel')
		net_WriteEntity(self)
		net_WriteString(item_id)
	net_Broadcast()
...
function meta:PS_SendPoints()
	net_Start('PS_Points')
		net_WriteEntity(self)
		net_WriteInt(self.PS_Points, 32)
	net_Broadcast()
end
...
function meta:PS_SendItems()
	net_Start('PS_Items')
		net_WriteEntity(self)
		net_WriteTable(self.PS_Items)
	net_Broadcast()
end

function meta:PS_SendClientsideModels()
	net_Start('PS_SendClientsideModels')
		net_WriteTable(PS.ClientsideModels)
	net_Send(self)
end
