...
function meta:StopNoise()
	local stsn = self:GetVoiceLines(VOICELINE_STOP)
	local zstsn = self:GetZombieClassTable().StopSounds
	for _, v in pairs(stsn) do
		self:StopSound(v)
	end
	if self:Team() == TEAM_UNDEAD and zstsn then
		for z, s in pairs(zstsn) do
			self:StopSound(s)
		end
	end
end
...
net.Receive("voice_stopnoise", function(len)
	local ent = net.ReadEntity()
	if ent:IsValid() then
		ent:StopNoise()
	end
