...
net.Receive("zs_remantleconf", function()
	...
	local disscraptxt = (gtbl.NoDismantle or not gtbl.QualityTier) and "Cannot Dismantle" or "Dismantle for " .. retscrap .. " Scrap"

	ri.m_Dismantle:SetText(disscraptxt)
	ri.m_Dismantle:SizeToContents()
	ri.m_Dismantle:CenterHorizontal()

	ri.m_DisaButton:SetDisabled(gtbl.NoDismantle or not gtbl.QualityTier)
	ri.m_DisaButton:SetTextColor((gtbl.NoDismantle or not gtbl.QualityTier) and COLOR_DARKGRAY or COLOR_WHITE)
end)
...
function GM:OpenRemantlerMenu(remantler)
	...
	sheet = remprop:AddSheet("Remantled Items", ammoframe, GAMEMODE.ItemCategoryIcons[ITEMCAT_AMMO], false, false)
	sheet.Panel:SetPos(0, tabhei + 2)
	...
	if not gtbl then
		...
		disabtn:SetDisabled(gtbl.NoDismantle or not gtbl.QualityTier)
	end
	disabtn:SetTextColor(gtbl and (gtbl.NoDismantle or not gtbl.QualityTier) and COLOR_DARKGRAY or gtbl and COLOR_WHITE or COLOR_DARKGRAY)
	frame.m_DisaButton = disabtn

	local disscraptxt = ""
	if gtbl then
		local retscrap = self:GetDismantleScrap(gtbl, SelectedInv())
		disscraptxt = (gtbl.NoDismantle or not gtbl.QualityTier) and "Cannot Dismantle" or "Dismantle for " .. retscrap .. " Scrap"
	end
