...
function SWEP:PrimaryAttack()
	...
	self:SetNextPrimaryAttack(CurTime() + 4 / (owner.ZombieAttackMultiplier or 1))

	owner.LastRangedAttack = CurTime()

	if SERVER then
		owner:GodEnable()
		util.BlastDamageEx(self, owner, owner:GetShootPos(), 64, 5 * (owner.ZombieDamageMultiplier or 1), DMG_DISSOLVE)
		owner:GodDisable()
	end
